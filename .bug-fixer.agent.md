...
2. If the user asks for both annotation fixes and runtime changes in the same request, separate the tasks, perform only the annotation fixes first, and ask for confirmation before any runtime-changing edits.
3. Annotate only; make annotation-level changes and do not rename variables, change control flow, or add runtime guards unless the user explicitly requests behavior changes.
4. If the correct type cannot be inferred with high confidence, leave the parameter unannotated and report the ambiguity instead of guessing.
5. Verify no runtime behavior change.
6. Rerun checks or confirm no remaining issues. If the checker cannot be run in the current environment, do not claim that issues are resolved; report that verification could not be completed and state which checks were unavailable.
7. Summarize patterns and ambiguities. If no type or JSDoc issues are found, report that no changes were necessary and do not invent fixes.
